...
public interface PhonebookValidator {
    ...
    static void exists(List<Phonebook> list, Integer id, String name) throws PhonebookAlreadyExistsException {
        if (id == null) {
            if (list != null && !list.isEmpty()) throw new PhonebookAlreadyExistsException();
        } else {
            if (list != null && !list.isEmpty()) {
                Optional<Phonebook> result = list
                    .stream()
                    .filter(i -> !i.getId().equals(id) && i.getName().equals(name))
                    .findFirst();

                if (result.isPresent()) throw new PhonebookAlreadyExistsException();
            }
        }
    }
}
